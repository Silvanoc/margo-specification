...
class OpenApiGenerator(Generator):
                ...
                if "requestBody" in req_spec and "content" in req_spec["requestBody"]:
                    for content_spec in req_spec["requestBody"]["content"].values():
                        if "$ref" in content_spec["schema"]:
                            resource_name = content_spec["schema"]["$ref"].removeprefix(
                                "#/components/schemas/"
                            )
                ...
                if "parameters" in req_spec:
                    for param_spec in req_spec["parameters"]:
                        if "$ref" in param_spec["schema"]:
                            resource_name = param_spec["schema"]["$ref"].removeprefix(
                                "#/components/schemas/"
                            )
                            result.add(resource_name)
                    ...
                    for response in req_spec["responses"].values():
                        if "content" in response:
                            for content_spec in response["content"].values():
                                if "$ref" in content_spec["schema"]:
                                    resource_name = content_spec["schema"][
                                        "$ref"
            ...
            for key, value in element.items():
                ...
                if isinstance(value, dict | list):
                    ...
                    if data_schema_name in self._renaming:
                        value = value.replace(
                            data_schema_name, self._renaming[data_schema_name]
                        )
    ...
    def _find_references(
        self, element: dict | list, referenced_data_schemas: set[str]
    ) -> None:
    ...
    def _sanitize_schemas(
        self, data_schemas: dict, endpoint_referenced_schemas: set[str]
    ) -> dict:
    ...
    def serialize(self, template_file: str = "", **kwargs) -> str:
        ...
        format_name = (
            getattr(self, "format", self.valid_formats[0]) or self.valid_formats[0]
        )
        ...
        if validator_class is None:
            raise ValueError(
                f"No validator available for OpenAPI version {expected_version}"
            )
        ...
        if not isinstance(self._template.get("components"), dict):
            raise ValueError(
                "OpenAPI template is missing required 'components' section"
            )
        endpoint_ref_schema_names = (
            self._find_referenced_schemas()
        )  # schemas referenced by OpenAPI endpoint(s)
        openapi_schemas = self._template["components"][
            "schemas"
        ...
        for endpoint_reference_name in endpoint_ref_schema_names:
            ...
            if linkml_element_name in self.schemaview.all_types().keys():
                all_req_data_schemas[linkml_element_name] = self._generate_type_schema(
                    linkml_element_name
                )
                continue
        ...
        # Keep schemas listed in the template even if not endpoint-referenced
        if self.keep_unreferenced:
            template_schema_names = set(self._template["components"]["schemas"].keys())
            template_schema_names.discard("{data_schema}")
            for tpl_schema_name in template_schema_names:
                raw_source = self._template["components"]["schemas"][
                    tpl_schema_name
                ].get("x-linkml-source", tpl_schema_name)
                # Resolve dotted path (class.slot) to synthetic $defs key
                linkml_element_name = raw_source
                if "." in raw_source and raw_source not in all_req_data_schemas:
                    parts = raw_source.split(".")
                    if len(parts) == 2:
                        synthetic_key = f"{parts[0]}__{parts[1]}_optional"
                        if synthetic_key in all_req_data_schemas:
                            linkml_element_name = synthetic_key
                if tpl_schema_name != linkml_element_name:
                    self._renaming[linkml_element_name] = tpl_schema_name
                directly_required_linkml_elements.add(linkml_element_name)
                if linkml_element_name in all_req_data_schemas:
                    continue
                if linkml_element_name in self.schemaview.all_types().keys():
                    all_req_data_schemas[linkml_element_name] = (
                        self._generate_type_schema(linkml_element_name)
                    )
                    continue
                json_schema = JsonSchemaGenerator(
                    self.schemaview.schema,
                ...
                json_defs = json.loads(json_schema.to_json())["$defs"]
                actual_key = [
                    k
                    for k in json_defs.keys()
                    if k.lower() == linkml_element_name.lower()
                ]
                effective_key = actual_key[0] if actual_key else linkml_element_name
                if effective_key in json_defs:
                    all_req_data_schemas[effective_key] = json_defs[effective_key]
        sanitized_data_schemas = self._sanitize_schemas(
            all_req_data_schemas, directly_required_linkml_elements
        )
        ...
        if self.inline_enums:
            sanitized_data_schemas = self._inline_enum_schemas(sanitized_data_schemas)
        ...
        if dangling:
            raise ValueError(
                f"Dangling $ref in generated OpenAPI spec: {','.join(dangling)}"
            )

        # Validate the generated output against the OpenAPI specification
        validate(result_obj, cls=validator_class)
        return result
    ...
    def printout_template(self) -> str:
        ...
        return openapi_generic_template.format(
            linkml_schema_id=self.schema.id, data_schema=first_element
        )
