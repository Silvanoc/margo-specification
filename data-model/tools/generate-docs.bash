...
for schema_name in "application-description" "application-deployment" "deployment-status" "desired-state-manifest" "device-capabilities"; do
  ${RUN} linkml generate doc \
    ...
    "${ROOT_DIR}/data-model/${schema_name}.linkml.yaml" >/dev/null
...
${RUN} linkml generate doc \
  ...
  --stacktrace \
  --example-directory="${ROOT_DIR}/data-model/resources/examples/valid" \
  "${ROOT_DIR}/data-model/margo-data-model.linkml.yaml" >/dev/null
...
# generate-openapi.bash writes to both generated/openapi/ and system-design/.
# Since system-design/ was already copied into merged/ above, copy the spec
# into merged/ now so mkdocs build can find it.
cp "${ROOT_DIR}/generated/openapi/workload-management-api-1.0.0.openapi.yaml" \
